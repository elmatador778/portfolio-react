...
export default function ChromaticFluid({ flowStrength = 0.8, grain = 0.04, contrast = 1.1, speed = 0.4, height = '100vh', style, children }) {
  const canvasRef = useRef(null);
  const hostRef = useRef(null);
  const settings = useMemo(() => ({ flowStrength, grain, contrast, speed }), [flowStrength, grain, contrast, speed]);
  ...
  return (
    <div ref={hostRef} style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: '#000000', width: '100%', height, ...style }}>
      <canvas ref={canvasRef} style={{ pointerEvents: 'none', position: 'absolute', inset: 0, display: 'block', width: '100%', height: '100%' }} />
